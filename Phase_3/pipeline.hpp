...
struct Instruction {
    ...
    // Cache and SPM related fields
    bool isSPM = false;
    bool isSync = false;
    int memoryLatency = 0;
    bool waitingForMemory = false;
    bool isInvalidateL1D=false;
};
...
public:
    Pipeline(bool enableForwarding = true) 
        : forwardingEnabled(enableForwarding), stallCount(0), instructionCount(0), memoryStallCycles(0) {
    ...
    int getMemoryStallCycles() const {
        return memoryStallCycles;
    }
    
    void incrementMemoryStallCycles(int cycles) {
        memoryStallCycles += cycles;
    }
    
    double getIPC() const {
        if (instructionCount == 0) return 0.0;
        int totalCycles = instructionCount + stallCount;
        return static_cast<double>(instructionCount) / totalCycles;
    }
