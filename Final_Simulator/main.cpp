...
int main() {
    ...
    // Configure cache
    std::cout << "\nProvide cache configuration file (leave empty for default): ";
    std::string cacheConfigFile;
    std::getline(std::cin, cacheConfigFile);
    cacheConfigFile = trim(cacheConfigFile);
    
    if (!cacheConfigFile.empty()) {
        simulator.loadCacheConfig(cacheConfigFile);
    }
